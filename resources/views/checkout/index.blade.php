...
<style>
  ...
  .select2-container--default .select2-selection--single { height: 38px; border-radius: .375rem; border: 1px solid #ced4da; }
  .select2-container--default .select2-selection--single .select2-selection__rendered { line-height: 38px; }
  .select2-container--default .select2-selection--single .select2-selection__arrow { height: 36px; }
  .select2-container .select2-selection--single { padding-left: .5rem; }
  .select2-container { width: 100% !important; }
  .select2-results__option { font-size: 14px; }
  .select2-selection__clear { margin-right: 6px; }
  label.form-label + .row .select2-container { margin-top: .25rem; }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // init select2
  $prov.select2({ placeholder: '-- Chọn tỉnh thành --', allowClear: true, width: '100%' });
  $ward.select2({ placeholder: '-- Chọn xã/phường --', allowClear: true, width: '100%' });

  // load provinces
  fetch('https://provinces.open-api.vn/api/v2/?depth=2')
    .then(r => r.json())
    .then(data => {
      data.forEach(p => { $('#checkout_province').append(`<option value="${p.code}">${p.name}</option>`); });
