    ...
    <div class="d-flex justify-content-between align-items-center mb-5 pb-3 border-bottom">
        <div>
            <h3 class="mb-1 font-weight-bold text-dark">
                <i class="fas fa-file-import text-primary mr-1"></i> Import Sản Phẩm
            </h3>
            <p class="text-muted mb-0">Tải lên file Excel để thêm nhiều sản phẩm cùng lúc</p>
        </div>
        <div>
            <a href="{{ route('admin.products.index') }}" class="btn btn-outline-secondary rounded px-3">
            ...
            <div class="card card-custom h-100">
                ...
                <div class="card-body pt-0">
                    <div class="row">
                        <div class="col-sm-7 pr-4 border-right">
                            ...
                            <ol class="list-unstyled pl-0 steps-list">
                                ...
                                <li class="mb-3">
                                    ...
                                    <p class="text-muted mb-1"><small>Điền <b>Tên sản phẩm</b>, <b>Danh mục</b>, <b>Giá</b>, <b>Số lượng</b> (bắt buộc) và <b>Mô tả</b> (tùy chọn).</small></p>
                                </li>
                                <li class="mb-3">
                        ...
                        <div class="col-sm-5 text-center d-flex flex-column justify-content-center align-items-center bg-light-soft rounded-lg p-3">
                            ...
                            <a href="{{ route('admin.products.import.template') }}" class="btn btn-success btn-md btn-block w-75 rounded">
                ...
                <div class="card-footer bg-light border-top rounded-bottom-lg">
                    ...
                    <ul class="mb-0 mt-2 pl-3" style="font-size: 0.85rem;">
                        ...
                        <li>Tên sản phẩm <b>không được trùng</b>.</li>
                        <li>Danh mục phải <b>tồn tại</b> trong hệ thống.</li>
                        <li>Giá và số lượng phải là <b>số hợp lệ</b>.</li>
                    </ul>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    uploadArea.on('dragover', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).css({'border-color': '#007bff', 'background': '#f0f8ff'});
    });
    
    uploadArea.on('dragleave', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $(this).css({'border-color': '#ced4da', 'background': 'white'});
    });
    ...
    $('#uploadForm').on('submit', function(e) {
        ...
        if (!fileInput.files.length) {
            Swal.fire({icon: 'warning', title: 'Chưa chọn file', text: 'Vui lòng chọn file Excel để tải lên!'});
            return;
        }
        ...
        data.forEach((row, index) => {
            ...
            tableHtml += `
                <tr class="${rowClass}" data-row-index="${index}">
                    ...
                    <td class="text-muted">${row.category_name || '<em class="text-muted">Trống</em>'}</td>
                    <td class="text-right">${row.price ? Number(row.price).toLocaleString('vi-VN') + 'đ' : '<em class="text-muted">0</em>'}</td>
                    <td class="text-center">${row.stock || '<em class="text-muted">0</em>'}</td>
                    <td class="text-muted"><small>${row.description || '<em class="text-muted">Không có</em>'}</small></td>
                    <td><img src="${row.image_url || ''}" alt="Hình ảnh" style="max-width: 50px; max-height: 50px; object-fit: cover; border-radius: 5px;"></td>
    ...
    function updatePreviewStats() {
        ...
        if (errors > 0) {
            ...
            $('#btnSave').prop('disabled', false).html(`<i class="fas fa-database mr-1"></i> Lưu ${valid} Sản Phẩm`);
        } else {
            $('#errorSection').slideUp();
    ...
    $('#btnSave').on('click', function() {
        ...
        if (validData.length === 0) {
            Swal.fire({icon: 'warning', title: 'Không có dữ liệu hợp lệ', text: 'Không có dòng dữ liệu nào hợp lệ để lưu!'});
            return;
        }
        
        Swal.fire({
            title: 'Xác nhận Import',
            html: `Bạn có chắc muốn import <strong>${validData.length}</strong> sản phẩm hợp lệ?`,
            icon: 'question',
            showCancelButton: true,
    ...
    function processImport(data) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                Swal.close();
                let message = 'Có lỗi xảy ra khi import!';
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    message = xhr.responseJSON.message;
                }
                Swal.fire({icon: 'error', title: 'Lỗi Hệ thống', text: message});
            }
        });
    }
});
