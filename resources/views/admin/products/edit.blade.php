...
<div class="shadow-sm rounded bg-white py-2">
    ...
    <div class="card m-3">
        ...
        <div class="card-body">
            <form action="{{ route('admin.products.update') }}" method="POST" enctype="multipart/form-data" id="productForm">
                @csrf
                @method('PUT')
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="0" {{ old('status', $product->status) == '0' ? 'selected' : '' }}>Tạm dừng</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row g-2" id="currentImages">
                                ...
                                <div class="col-6 current-image" data-image-id="{{ $image->id }}">
                                    <div class="position-relative">
                                        <img src="{{ $image->image_url }}" alt="Product Image" 
                                             class="img-fluid rounded" style="height: 80px; width: 100%; object-fit: cover;">
                                        <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 remove-current-image" 
                            ...
                            <div class="dropzone-area" id="productImageDropzone">
                                <div class="dz-message">
                                    <div class="text-center">
                                        ...
                                        <p class="text-muted mb-0">Kéo thả hình ảnh vào đây</p>
...
.preview-image img, .current-image img {
    width: 100%;
    height: 80px;
    object-fit: cover;
}
...
.remove-image:hover, .remove-current-image:hover {
...
$(document).ready(function() {
    ...
    function uploadFile(file) {
        ...
        console.log('Edit page - Upload URL:', uploadUrl);
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 404) {
                    message = 'Không tìm thấy đường dẫn upload (404)';
                } else if (xhr.status === 500) {
                    message = 'Lỗi server (500)';
                } else if (xhr.responseJSON && xhr.responseJSON.message) {
                    message = xhr.responseJSON.message;
                } else if (xhr.responseText) {
                    message = 'Lỗi: ' + xhr.responseText.substring(0, 100);
                }
    ...
    window.removeImage = function(filename) {
        // Cleanup ảnh ngay lập tức
        cleanupTempImages([filename]);
    ...
    $('.remove-current-image').on('click', function() {
        ...
        $(`.current-image[data-image-id="${imageId}"]`).fadeOut(300, function() {
            $(this).remove();
        });
    });
    ...
    $('button[type="reset"]').on('click', function() {
        ...
        // Show back removed current images
        $('.current-image').show();
    });
